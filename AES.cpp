...
{  
    ...
    cin>>plaintext;
    int total_blocks = ceil(plaintext.length()/16.0);
    byte plain_total[total_blocks][16];
    byte plain[16]; 
    for(int j = 0;j<total_blocks;j++)
    {
        for(int i = 0;i<16;i++)
        {
            plain[i]=bitset<8>(plaintext[i+j*16]);
            plain_total[j][i]=bitset<8>(plaintext[i+j*16]);
        }
    }
    byte key[16];
    string keytext;
   ...
   for(int j = 0;j<total_blocks;j++){
    for(int i=0; i<16; ++i)  
    {  
        cout << hex<<plain_total[j][i].to_ulong() << " ";  
        if((i+1)%4 == 0)  
            cout << endl;  
    }  
   ...
   for(int j = 0;j<total_blocks;j++){
    ...
    cout << endl;  
    cout<<"Decrypted cyphertext:\n";
    for(int j = 0;j<total_blocks;j++){
    decrypt(plain_total[j], w);  
    for(int i=0; i<16; ++i)  
    {  
        cout << hex<<plain_total[j][i].to_ulong() << " ";  
        if((i+1)%4 == 0)  
    ...
    for(int i=0; i<16; ++i)  
    {  
        unsigned long j1 = plain_total[j][i].to_ulong();
        unsigned char c = static_cast<unsigned char>(j1);
        cout << c;  
    }  
    cout << endl;  
    return 0;  
